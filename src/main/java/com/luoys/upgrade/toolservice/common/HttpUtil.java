...
//    }
    ...
    public static boolean execute(HttpRequestDTO httpRequestDTO) {
        ...
        if (httpRequestDTO.getHttpHeaderList() != null || httpRequestDTO.getHttpHeaderList().size() > 0) {
            for (ParamDTO paramDTO : httpRequestDTO.getHttpHeaderList()) {
                header.put(paramDTO.getName(), paramDTO.getValue());
            }
        }
        switch (httpRequestDTO.getHttpType()) {
            case "":
        }
        restTemplate.exc.execute(httpRequestDTO.getHttpURL(), HttpMethod.POST, )
    }
